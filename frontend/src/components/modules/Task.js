      ...
      <div className="container taskStyle">
        ...
        <div className="row addContent gap-2">
          ...
          <input
            ...
            type="text"
            name="taskinp"
            onKeyUp={(e) => this.keyHandler(e, "inp")}
            placeholder="Enter the task"
            value={this.state.taskinp}
            onChange={(e) => this.handleInp(e)}
          />
          <button className="col btn-grad" onClick={this.addTask}>
            Add
          </button>
          <button
        ...
        {this.state.taskList.length === 0 ? (
          <h5 className="my-4 text-center">No Task Added</h5>
        ) : (
                    ...
                    {this.state.editEnable && this.state.editId === item.id ? (
                      <td colSpan={2}>
                        <input
                          ...
                          type="text"
                          name="editValue"
                          onKeyUp={(e) => this.keyHandler(e, "edit", item.id)}
                    ...
                    {this.state.editEnable && this.state.editId === item.id ? (
                      ...
                      <td>
                        <select
                          ...
                          id=""
                          value={item.status}
                          onChange={(e) => this.statusUpdate(item.id, e)}
                        >
                    ...
                    <td>
                      <div className="pt-2 d-flex align-items-center justify-content-evenly">
                        ...
                        this.state.editId === item.id ? (
                          <Icon.CheckSquare
                            onClick={() => this.submitEdit(item.id)}
                            data-bs-toggle="tooltip"
                            title="save"
                          />
                        ) : (
                          <Icon.PencilSquare
                            onClick={() => this.enableEdit(item.id)}
                            data-bs-toggle="tooltip"
                            title="edit"
                            style={
                              item.status === "completed"
