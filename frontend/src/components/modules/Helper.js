...
export default class Helper extends Component {
  ...
  createBullet = () => {
    if (this.inpvalid()) {
      ...
      for (let i = 0; i < strArr.length; i++) {
        ...
        if (charTxt.match(/[0-9]/i) && !olFlag) olFlag = true;
        if (charTxt.match(/[a-zA-Z0-9]/i))
          trimTxt = trimTxt.substring(trimTxt.indexOf(".") + 1);
        else trimTxt = trimTxt.substring(1);
      ...
      for (let i = 0; i < strArr.length; i++) {
        ...
        if (tag === "img") {
          ...
          if (i === 0)
            ...
            trimTxt = `<option value="${trimTxt}">${trimTxt}</option>\n</select>`;
      ...
      for (let i = 0; i < len; i++) {
        if (strArr[i].indexOf(`<${tag}`) >= 0) {
          ...
          strArr[i] = strArr[i].substring(
            strArr[i].indexOf(">", strArr[i].indexOf(`<${tag}`)) + 1
          );
  ...
  addInsertPos = () => {
    ...
    let newText =
      ...
      textToInsert +
      textArea.value.substring(endPos, textArea.value.leength);
    this.setState({
      inpDialog: newText,
    ...
    return (
      <div className="container container-fluid">
        <div className="row my-3 justify-content-space-between">
          ...
          <button
            className="col-md-3 btn-grad"
            data-bs-toggle="collapse"
            data-bs-target="#opContent"
            onClick={() =>
              this.setState({ arrowRotate: !this.state.arrowRotate })
            }
          >
            Operations{" "}
            <Icon.CaretRightFill
              className={this.state.arrowRotate ? "rotateRight" : "arrow"}
            />
          </button>
        </div>
        <div className="collapse navbar-collapse" id="opContent">
          ...
          <div className="row mb-3 gap-2">
            <button className="col btn-grad" onClick={this.removeNbspFunc}>
              Remove nbsp
            </button>
            <button
              className="col btn-grad"
              onClick={() => this.setState({ showDialogBox: true })}
            >
              Convert txt to structure
            </button>
            <button className="col btn-grad" onClick={this.createBullet}>
              Create Bullet point
            </button>
          </div>
          <div className="row mb-3 gap-2">
            <input
              type="text"
              className="col-md-8"
              onKeyUp={(e) => this.keyHandler(e, "rmtag")}
              placeholder="Enter the html tag to be added/removed eg: sup"
              name="rmtag"
              value={this.state.rmtag}
              onChange={(e) => this.inpTxtHandler(e)}
              id=""
            />
            <button className="col btn-grad" onClick={this.addTags}>
              Add tags
            </button>
            <button className="col btn-grad" onClick={this.removeTags}>
              Remove tags
            </button>
          </div>
          <div className="row mb-3 gap-2">
            ...
            <input
              ...
              placeholder="Replace"
              name="replStr"
              value={this.state.replStr}
              onChange={(e) => this.inpTxtHandler(e)}
              id=""
            />
            <button className="col-md btn-grad" onClick={this.replaceStr}>
              Replace String
            </button>
          </div>
        </div>
          ...
          <div className="dialogBoxArea">
            ...
            <div className="row gap-3 mt-4">
              <textarea
                ...
                cols="30"
                rows="10"
                placeholder="Enter the structure eg.:"
                value={this.state.inpDialog}
                onChange={(e) => this.inpTxtHandler(e)}
              ></textarea>
              <button className="col-md btn-grad" onClick={this.addInsertPos}>
                Add input
              </button>
              <button
                className="col-md btn-grad"
                onClick={this.createStructureFromTxt}
              >
                Done
              </button>
              <button
        ...
        <div className="row gap-3 io-area">
          <h5>
            Input <Icon.ChevronCompactDown />
          </h5>
          <textarea
            name="inpTxt"
            id=""
            placeholder="Enter your input here and click above operation to perform"
            cols="30"
            rows="10"
          ...
          <h5 id="output">
            Output <Icon.ChevronCompactDown />
          </h5>
          <textarea
